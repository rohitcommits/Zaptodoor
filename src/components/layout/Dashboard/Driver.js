...
const OnOffBadge = ({ value, theme }) => {
  ...
  return (
    <span style={{ padding: "3px 10px", borderRadius: "12px", fontSize: "11px", fontWeight: 700, background: c.bg, color: c.color }}>
      {value}
    </span>
  );
};
...
const DriverTable = ({ isDark = true }) => {
  ...
  // Navigate to Add New Driver page for new driver
  const handleAddDriverNavigate = () => {
    navigate("/Dashboard/AddNewDriver", { state: { mode: "add" } });
  };

  // Navigate to Add New Driver page for editing existing driver
  const handleEditDriverNavigate = (driver) => {
    navigate("/Dashboard/AddNewDriver", { state: { mode: "edit", driverData: driver } });
  };
  ...
  const handleVerifyDocuments = (driver) => {
  ...
  const handleViewEarnings = async (driver) => {
    ...
    try {
      ...
      setDriverEarnings(earnings || { 
        ...
        breakdown: { weekly: "₹8,500", monthly: "₹32,000", yearly: "₹1,85,000" } 
      });
    } catch (err) { 
      setDriverEarnings({ 
        total: driver.earnings, 
  ...
  const handleViewWallet = async (driver) => {
    ...
    try {
      ...
      setDriverWallet(wallet || { 
        ...
        transactions: [
          { date: "2024-02-01", amount: "+₹5,000", type: "credit" }, 
          { date: "2024-01-28", amount: "-₹2,500", type: "debit" }
        ] 
      });
    } catch (err) { 
      setDriverWallet({ 
  ...
  const handleViewHistory = async (driver) => {
    ...
    try {
      ...
      setDriverHistory(history || [
        { id: "DEL-001", date: "2024-02-15", amount: "₹250", status: "completed", restaurant: "Pizza Hut", customer: "John Doe" },
        { id: "DEL-002", date: "2024-02-14", amount: "₹180", status: "completed", restaurant: "McDonald's", customer: "Jane Smith" },
        { id: "DEL-003", date: "2024-02-13", amount: "₹320", status: "cancelled", restaurant: "KFC", customer: "Bob Wilson" }
      ]);
    } catch (err) { 
      setDriverHistory([
        { id: "DEL-001", date: "2024-02-15", amount: "₹250", status: "completed", restaurant: "Pizza Hut", customer: "John Doe" },
  ...
  const exportToCSV = () => {
    ...
    const csvData = mappedDrivers.map(d => [
      d.sn, d.driverId, d.status, d.date, d.ready, d.onOff, 
      d.name, d.contact, d.verified ? "Yes" : "No", d.wallet
    ]);
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.headerActions}>
        <button style={styles.addBtn} onClick={handleAddDriverNavigate}>
          <PlusIcon /> Add Driver
        </button>
      ...
      <div style={styles.filtersBar}>
        <div style={styles.searchWrapper}>
          ...
          <input type="text" style={styles.searchInput} placeholder="Search drivers..." value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
              ...
              {Object.entries(selectedDriver.documents || { aadhar: "Pending", license: "Pending", rc: "Pending" }).map(([doc, status]) => (
                <div key={doc} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: `1px solid ${colors.border}` }}>
                  ...
                  {status === "Verified" ? (
                    ...
                    <button onClick={() => verifyDocument(doc)} style={{ padding: "4px 12px", borderRadius: "6px", background: "#3b82f6", color: "white", border: "none", cursor: "pointer", fontSize: "11px" }}>
                      Verify
                    </button>
                  )}
                </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ borderBottom: `1px solid ${colors.border}` }}>
                  ...
                  <th style={{ textAlign: "left", padding: "10px", fontSize: "12px", color: "#3b82f6" }}>Customer</th>
                  <th style={{ textAlign: "left", padding: "10px", fontSize: "12px", color: "#3b82f6" }}>Amount</th>
                  <th style={{ textAlign: "left", padding: "10px", fontSize: "12px", color: "#3b82f6" }}>Status</th>
                </tr>
              </thead>
              <tbody>
