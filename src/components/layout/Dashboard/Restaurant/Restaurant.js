...
const DUMMY_RESTAURANTS = [
  ...
  },
  {
    ...
    completedPayments: 200000,
    address: "789 Thai Street",
    email: "contact@thaispice.com",
    description: "Authentic Thai cuisine",
    cuisine: "Thai, Asian",
    menu: [
      { id: 1, name: "Pad Thai", category: "Noodles", price: 249, description: "Stir-fried rice noodles", isAvailable: true },
      { id: 2, name: "Tom Yum Soup", category: "Soups", price: 149, description: "Spicy and sour soup", isAvailable: true },
    ],
    schedule: [
      ...
      { day: "Saturday", openTime: "12:00", closeTime: "23:00", isClosed: false },
...
// ─────────────────────────────────────────────────────────────────────────────
// MAIN COMPONENT - RestaurantTable
// ─────────────────────────────────────────────────────────────────────────────
const RestaurantTable = ({ isDark = true }) => {
  ...
  const handleExport = (exportConfig) => { 
    ...
    const headers = selectedFields.map(f => ({ id: "ID", restroId: "Restro ID", name: "Name", contact: "Contact", status: "Status", ready: "Ready", onOff: "ON/OFF", shiftStart: "Shift Start", shiftEnd: "Shift End", rating: "Rating", totalPayments: "Total Payments" }[f] || f)); 
    const csvData = mappedRestaurants.map(r => selectedFields.map(f => r[f] || '').join(',')); 
  ...
  // If showing detail page, render RestaurantDetailPage component
  if (showDetailPage) {
    return (
    ...
    <div style={styles.container}>
      ...
      <div style={styles.filtersBar}>
        <div style={styles.searchWrapper}>
          ...
          <input type="text" style={styles.searchInput} placeholder="Search by name, ID, or contact..." value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
    ...
    {loading ? (
      ...
      mappedRestaurants.map((restro) => (
        <tr
          key={restro.id}
          style={{
            background: selected.has(restro.id)
        ...
        >
          <td style={styles.td}>
            <Checkbox
              checked={selected.has(restro.id)}
              onChange={() => toggleOne(restro.id)}
            />
          </td>
          ...
          <td style={styles.td}>
            <AvatarPlaceholder name={restro.name} />
          </td>

          <td style={styles.td}>
            <StatusBadge status={restro.status} />
          </td>

          <td style={styles.td}>
            <ReadyBadge ready={restro.ready} />
          </td>

          <td style={styles.td}>
            <OnOffBadge value={restro.onOff} />
          </td>
          ...
          <td style={styles.td}>
            ...
            >
              <PaymentIcon /> ₹
              {restro.totalPayments?.toLocaleString()}
            </div>
          </td>
          ...
          <td style={styles.td}>
            <button
              ...
              onClick={() => setShowDetailPage(restro)}
            >
              View More
            </button>
          </td>
